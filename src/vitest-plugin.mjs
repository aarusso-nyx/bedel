// Adapted for Bedel: portable dependency lookup, plugin identity, explicit test selection.
import { createRequire } from "node:module";
import { dirname, isAbsolute, join } from "node:path";
import { sandboxWorkspaceAliases } from "./workspace-aliases.mjs";
import { pathToFileURL } from "node:url";

// Standalone adapter loaded in Stryker workers. Candidate configuration supplies
// aliases/setup/environment; explicit testFiles determines each unit population.
const require = createRequire(import.meta.url);
const base = dirname(
  require.resolve("@stryker-mutator/vitest-runner/package.json"),
);
  ...
  return createVitestWithRunnerLoader(
    ...
    { ...options, configLoader: "runner" },
    {
      ...
      plugins: [
        ...(inline.plugins ?? []),
        {
          name: "bedel-sandbox-workspace",
          enforce: "pre",
          config: () => ({
            resolve: { alias: sandboxWorkspaceAliases(process.cwd()) },
          }),
        },
      ],
    },
...
const coreRequire = createRequire(
  require.resolve("@stryker-mutator/core/package.json"),
);
...
function createProtectedVitest(injector) {
  ...
  async function tracePhase(phase, operation) {
    ...
    const emit = (status) =>
      runner.log.error(
        `BEDEL_RUNNER_PHASE ${JSON.stringify({
          version: 1,
          pid: process.pid,
          sequence,
          phase,
  ...
  runner.init = function () {
    return tracePhase("init", async () => {
      ...
      if (
        !this.ctx ||
        !Array.isArray(this.ctx.projects) ||
        this.ctx.projects.length === 0
      )
