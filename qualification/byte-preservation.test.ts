// Ported from DEVAI 18fc6cf0: assert the standalone runner byte-preservation setting.
...
it("keeps pinned vendor bytes exact through the real Stryker preprocessor", async () => {
  ...
  const { DisableTypeChecksPreprocessor } = await import(
    pathToFileURL(
      join(core, "dist/src/sandbox/disable-type-checks-preprocessor.js"),
    ).href
  );
  ...
  const source = readFileSync("src/runner.mjs", "utf8");
  const setting = /disableTypeChecks:\s*(false)/u.exec(source)?.[1];
