// Ported from DEVAI 18fc6cf0, adapter lookup and plugin name changed for Bedel.
...
interface MutationReport {
  files: Record<
    string,
    { mutants: { static?: boolean; status: string; statusReason?: string }[] }
  >;
...
function runFixture(
  ...
  try {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        name: "static-activation-fixture",
        private: true,
        type: "module",
      }),
    );
    ...
    const plugin = readFileSync(join(repo, "src/vitest-plugin.mjs"), "utf8");
    expect(plugin).toContain("createRequire(import.meta.url)");
    writeFileSync(
    ...
    writeFileSync(
      join(root, "workspace-aliases.mjs"),
      readFileSync(join(repo, "src/workspace-aliases.mjs")),
    );
    writeFileSync(
    ...
    inspect(
      root,
      result.status,
      readFileSync(join(root, "runner.log"), "utf8"),
    );
    passed = true;
  } finally {
...
`;
  runFixture(
    ...
    (root, status) => {
      ...
      expect(
        mutants.filter((mutant) => mutant.static).length,
      ...
      expect(
        Object.values(report.testFiles).map((file) => file.tests.length),
      ).toEqual([6, 6]);
    },
  );
}, 60000);
    ...
    runFixture(source, population, ["value.js:1-1"], (root, status, log) => {
      if (!ready) {
        ...
        expect(log).toContain(
          "A test suite failed during the unmutated baseline.",
        );
        return;
      }
      expect(status, `Retained fixture: ${root}`).toBe(0);
